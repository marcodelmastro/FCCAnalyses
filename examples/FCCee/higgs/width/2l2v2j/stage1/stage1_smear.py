...
outputDir   = "outputs/fccee/higgs/mH-recoil/neutrinos/stage1/smear_sf_2_5/"
...
class RDFanalysis():
    ...
    def analysers(df):
        ...
        collections_res["PFParticles"] = "ReconstructedParticles2p"
        ...
        df2 = (
            df
            .Alias("mc_reco_0", "{}#0.index".format(collections["MCRecoMap"]))
            .Alias("mc_reco_1", "{}#1.index".format(collections["MCRecoMap"]))
            # matching between the RecoParticles and the MCParticles:
            .Define("reco_mc_index","ReconstructedParticle2MC::getRP2MC_index(mc_reco_0,mc_reco_1,{})".format(collections["PFParticles"]))


            .Define(collections_res["PFParticles"],ROOT.SmearObjects.SmearedReconstructedParticle(2.5, 130, 0, False),[collections["PFParticles"], "reco_mc_index", collections["GenParticles"]])
            ...
            .Define("muons",                "ReconstructedParticle::get(Muon0, ReconstructedParticles2p)")
            .Define("electrons",            "ReconstructedParticle::get(Electron0, ReconstructedParticles2p)")
...
#puis on crée des pseudo-jets avec 
            ...
            .Define("my_recoparticles",  "ReconstructedParticle::remove(ReconstructedParticles2p, taken_leptons)")
...
#il faut faire pareil que pour Z mais pour Z1 et Z2 (l'un après l'autre c'est ok je pense à chaque fois faut juste prendre la liste qui a subi les sélections) , mais là attention il faut faire un sel_pdgID avec des sortes de où pour avoir tous les quarks/gluon : écrire une fonction ? en se basant sur sel_pdgID mais en rajoutant + d'arguments en entrée et un ou dans les condit
            ...
            .Define("new_visible_4vector", "ReconstructedParticle::get_P4vis(ReconstructedParticles2p)")

            
            .Define("new_visible_mass_predef", "new_visible_4vector.M()")
            .Define("px", "new_visible_4vector.Px()")
            .Define("py", "new_visible_4vector.Py()")
            .Define("pz", "new_visible_4vector.Pz()")

            .Define("newemiss", "sqrt(px*px + py*py + pz*pz)")
            .Define("newetmiss", "sqrt(px*px + py*py)")
            .Define("newpxmiss", "px*(-1)")
            .Define("newpymiss", "py*(-1)")
            .Define("newpzmiss", "pz*(-1)")

            .Define("new_missing_tlv", "ReconstructedParticle::get_tlv_easy(newemiss, newpxmiss, newpymiss, newpzmiss)")
